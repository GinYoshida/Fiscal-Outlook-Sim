...
│   ├── ui/
│   │   ├── Slider.tsx        # 汎用スライダーコンポーネント
│   │   ├── NumberInput.tsx   # 汎用数値入力コンポーネント
...
│
├── styles/
│   ├── base.css              # リセット・CSS変数・ページ基本
│   ├── layout.css            # タブ・グリッド・モバイルトグル
│   ├── sidebar.css           # サイドバー・スライダー・最適化
│   ├── charts.css            # チャート・データテーブル・ヘルプ
│   ├── scenarios.css         # シナリオ一覧タブ
│   └── components.css        # 汎用UI（折りたたみ・警告・解説パーツ）
│
├── svg/
│   └── FlowDiagram.tsx       # 統合政府フロー図（SVG）
│
└── _baseline.ts              # ベースライン数値検証用
```
...
**コメント方針:**
- 全 `.ts` / `.tsx` ファイルの冒頭に `/** ... */` 形式でファイルヘッダー（日本語）
- export される関数・コンポーネント・インターフェースにJSDoc
- `simulation.ts` はロジックツリー A〜J のセクションコメントで区分
- ハードコード定数に経済的根拠・出典を記載
- CSSファイル冒頭にセクション説明コメント
...
- **3タブ構成:**
    ...
    3.  **シナリオ一覧タブ:** 10種類のプリセットシナリオ（ベースライン、高成長、スタグフレーション、金利急騰、財政再建、急激円安、少子化加速、テクノロジー革命、グローバル危機、再分配強化）を横断的に比較し、棒グラフ、レーダーチャート、折れ線推移、サマリー表、展開式カード形式で表示します。シナリオ比較は30年固定で実行。好結果4シナリオ（②⑤⑧⑩）はB+〜A+評価、リスク5シナリオ（③④⑥⑦⑨）はD評価、①ベースラインは現状維持の厳しさを示すD評価。⑩は消費税10%据え置き・所得税/法人税累進強化・内部留保還元のみで再建を目指すシナリオ。
...
-   **URL共有機能:** パラメータの状態をURLクエリパラメータにシリアライズし、「共有URLをコピー」ボタンでリンクを生成。デフォルト値と異なるパラメータのみを短縮キーで含めることでURL長を最適化。
